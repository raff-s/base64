...
fn _calc_decoded_len(input: []const u8) !usize {
    if (input.len < 4) {
        return 3;
    }

    const n_output = try std.math.divFloor(usize, input.len, 4);
...
test "base64._calc_decoded_len" {
    try std.testing.expectEqual(try _calc_decoded_len(""), 3);
    try std.testing.expectEqual(try _calc_decoded_len("aa"), 3);
    try std.testing.expectEqual(try _calc_decoded_len("aaa"), 3);
    try std.testing.expectEqual(try _calc_decoded_len("aaaaaaaa"), 6);
}
